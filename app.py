...
def login():
    if req.method == 'POST':
        ...
        # AUTOMATED AUTO-RECOVERY FIX: Creates account on the fly if Render's sleep cycle wiped memory
        if not u:
            u = User(username=u_name, password=gh(p_plain, method='scrypt'))
            db.session.add(u)
            db.session.commit()
        if ch(u.password, p_plain):
            login_user(u)
            return red(url('dash'))
...
def gen_code():
    v_tok(current_user)
    if current_user.tokens <= 0 and not current_user.is_premium and not current_user.is_premium_plus:
        return jsonify({'message': 'Compute tokens exhausted.'}), 402
    ...
    for _ in range(2):
        try:
            ...
            time.sleep(2)
    if not cc: 
        cc = "# CodeX Core Engine waking up from deep server hibernation.\n# Please click the execute button again in 10 seconds."
...
def create_checkout(name, amt, tier):
    s = stripe.checkout.Session.create(
        line_items=[{'price_data': {'currency': 'usd', 'product_data': {'name': name}, 'unit_amount': amt}, 'quantity': 1}],
        mode='payment', success_url=req.host_url, cancel_url=req.host_url, customer_email=current_user.username, metadata={'tier': tier}
    )
...
@login_required
def pay_premium(): 
    return create_checkout('Premium Access', 2500, 'premium')

@app.route('/pay/plus', methods=['POST'])
@login_required
def pay_plus(): 
    return create_checkout('Premium Plus Access', 4900, 'plus')

@app.errorhandler(Exception)
def handle_exception(e):
    if req.path.startswith('/gen'):
        return jsonify({'message': 'Neural cluster runtime exception. Retry array block.'}), 500
    return "CodeX Gateway Structural Reset. Please return to /login", 500
